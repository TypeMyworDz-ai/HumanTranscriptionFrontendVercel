...
import { approveHumanOrder, approveHumanSubmission, claimHumanJob, createHumanOrder, listAdminQueue, listClientOrders, listOpenJobs, listSubmittedTranscripts, recordPreviewPayment, submitHumanTranscript } from './humanData';
import './App.css';
...
function ClientPortal({ session, profile, demo }) {
  ...
  const saveDraft = async () => {
    ...
    try {
      const created = await createHumanOrder({ clientId: profile.id, file, durationMinutes, serviceType, rush, difficulty, speakerCount, timestamps, formattingNotes, quote });
      setOrderId(created.id);
      setSaveState('Brief saved. Choose Kora or Paystack below to complete the preview checkout.');
  ...
  const completePreviewPayment = async () => {
    if (!orderId) return;
    setPaymentBusy(true); setSaveState('');
    try {
      await recordPreviewPayment(orderId, paymentMethod);
      setOrderId(null);
      setSaveState(`Preview ${paymentMethod === 'kora' ? 'Kora' : 'Paystack'} payment recorded. The order is now waiting for Admin review.`);
      const refreshed = await listClientOrders(profile.id);
      setRealOrders(refreshed.data || []);
    } catch (error) { setSaveState(`Could not complete preview payment: ${error.message}`); }
